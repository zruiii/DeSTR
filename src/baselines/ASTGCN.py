...
class Spatial_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        :return: (B,N,N)
        '''
        lhs = torch.einsum('ijkl,l->ijk', x, self.W1)
        lhs = torch.einsum('ijk,kl->ijl', lhs, self.W2)

        rhs = torch.einsum('k,ijkl->ijl', self.W3, x).transpose(-1, -2)
...
class Temporal_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        rhs = torch.einsum("i,jkil->jkl", self.U3, x)
...
def make_astgcn(DEVICE, adj_mx, num_for_predict, len_input, num_of_vertices, nb_block=2, in_channels=1, K=3, nb_chev_filter=64, nb_time_filter=64, time_strides=1):
    '''
    :param DEVICE:
    :param nb_block:
    :param in_channels:
